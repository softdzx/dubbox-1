...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
    ...
    public boolean isAvailable() {
        ...
        return null != invoker ? invoker.isAvailable() : directory.isAvailable();
    }
            ...
            try {
                ...
                if (rinvoker != null) {
                    ...
                    try {
                        ...
                        LogHelper.warn(logger, e.getMessage() + " may because invokers list dynamic change, ignore.", e);
                    }
                }
            } catch (Throwable t) {
                LogHelper.error(logger, "clustor relselect fail reason is :" + t.getMessage()
    ...
    private Invoker<T> reselect(LoadBalance loadbalance, Invocation invocation,
        ...
        //预先分配一个，这个列表是一定会用到的.
        List<Invoker<T>> reselectInvokers = Lists.newArrayListWithCapacity(invokers.size() > 1 ? (invokers.size() - 1) : invokers.size());
