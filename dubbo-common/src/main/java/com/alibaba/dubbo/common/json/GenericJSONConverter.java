...
public class GenericJSONConverter implements JSONConverter {
    ...
    private static final Map<Class<?>, Encoder> GlobalEncoderMap = Maps.newHashMap();

    private static final Map<Class<?>, Decoder> GlobalDecoderMap = Maps.newHashMap();
    ...
    public void writeValue(Object obj, JSONWriter jb, boolean writeClass) throws IOException {
        ...
        if (encoder != null) {
            ...
            for (String pn : pns) {
                if ((obj instanceof Throwable) && ("localizedMessage".equals(pn)
                        || "cause".equals(pn) || "stackTrace".equals(pn))) {
                    continue;
                }
