...
public class RegistryDirectory<T> extends AbstractDirectory<T> implements NotifyListener {
    ...
    public void destroy() {
        ...
        try {
            ...
            LogHelper.warn(logger, "unExpected error when unSubscribe service " + serviceKey + "from registry"
                    + registry.getUrl(), t);
        }
        ...
        try {
            ...
            LogHelper.warn(logger, "Failed to destroy service " + serviceKey, t);
        }
    }

    public synchronized void notify(List<URL> urls) {
        List<URL> invokerUrls = Lists.newArrayList();
        List<URL> routerUrls = Lists.newArrayList();
        List<URL> configuratorUrls = Lists.newArrayList();
        for (URL url : urls) {
            ...
            if (Constants.ROUTERS_CATEGORY.equals(category)
                ...
                LogHelper.warn(logger, "Unsupported category " + category + " in notified url: " + url
    ...
    private void refreshInvoker(List<URL> invokerUrls) {
        if (invokerUrls != null && invokerUrls.size() == 1 && invokerUrls.get(0) != null
            ...
            if (invokerUrls.size() == 0 && this.cachedInvokerUrls != null) {
                ...
                this.cachedInvokerUrls = Sets.newHashSet();
            ...
            // state change
            //如果计算错误，则不进行处理.
            if (CollectionUtils.isEmpty(newUrlInvokerMap)) {
                LogHelper.error(logger, new IllegalStateException("urls to invokers error .invokerUrls.size :" + invokerUrls.size()
            ...
            try {
                ...
                LogHelper.warn(logger, "destroyUnusedInvokers error. ", e);
            }
        }
    }

    private Map<String, List<Invoker<T>>> toMergeMethodInvokerMap(Map<String, List<Invoker<T>>> methodMap) {
        Map<String, List<Invoker<T>>> result = Maps.newHashMap();
        methodMap.forEach((method, invokers) -> {
            Map<String, List<Invoker<T>>> groupMap = Maps.newHashMap();
            for (Invoker<T> invoker : invokers) {
                String group = invoker.getUrl().getParameter(Constants.GROUP_KEY, "");
                List<Invoker<T>> groupInvokers = groupMap.computeIfAbsent(group, k -> Lists.newArrayList());
                groupInvokers.add(invoker);
            }
            if (groupMap.size() == 1) {
                result.put(method, groupMap.values().iterator().next());
            } else if (groupMap.size() > 1) {
                List<Invoker<T>> groupInvokers = Lists.newArrayList();
     ...
     * @return configurator list
     */
    public static List<Configurator> toConfigurators(List<URL> urls) {
        List<Configurator> configurators = Lists.newArrayListWithCapacity(urls.size());
        if (urls.size() == 0) {
        ...
        for (URL url : urls) {
            ...
            //override 上的any host可能是自动添加的，不能影响改变url判断
    ...
    private List<Router> toRouters(List<URL> urls) {
        List<Router> routers = Lists.newArrayList();
        ...
        for (URL url : urls) {
            ...
            try {
                ...
                LogHelper.error(logger, "convert router url to router error, url: " + url, t);
            }
        }
        return routers;
    }
    ...
    private Map<String, Invoker<T>> toInvokers(List<URL> urls) {
        Map<String, Invoker<T>> newUrlInvokerMap = Maps.newHashMap();
        if (CollectionUtils.isEmpty(urls)) {
        ...
        for (URL providerUrl : urls) {
            ...
            if (!Strings.isNullOrEmpty(queryProtocols)) {
            ...
            if (!ExtensionLoader.getExtensionLoader(Protocol.class).hasExtension(providerUrl.getProtocol())) {
                LogHelper.error(logger, new IllegalStateException("Unsupported protocol " + providerUrl.getProtocol() +
                        " in notified url: " + providerUrl + " from registry " + getUrl().getAddress() + " to consumer "
            ...
            if (invoker == null) { // 缓存中没有，重新refer
                try {
                    ...
                    LogHelper.error(logger, "Failed to refer invoker for interface:" + serviceType
    ...
    private URL mergeUrl(URL providerUrl) {
        ...
        if (Strings.isNullOrEmpty(providerUrl.getPath())
    ...
    private Map<String, List<Invoker<T>>> toMethodInvokers(Map<String, Invoker<T>> invokersMap) {
        Map<String, List<Invoker<T>>> newMethodInvokerMap = Maps.newHashMap();
        // 按提供者URL所声明的methods分类，兼容注册中心执行路由过滤掉的methods
        List<Invoker<T>> invokersList = Lists.newArrayList();
        if (!CollectionUtils.isEmpty(invokersMap)) {
                ...
                if (!Strings.isNullOrEmpty(parameter)) {
                    ...
                    if (!CollectionUtils.isEmpty(methods)) {
                        for (String method : methods) {
                            if (!Strings.isNullOrEmpty(method) && !Constants.ANY_VALUE.equals(method)) {
                                List<Invoker<T>> methodInvokers = newMethodInvokerMap.computeIfAbsent(method, k -> Lists.newArrayList());
        ...
        if (!CollectionUtils.isEmpty(serviceMethods)) {
            for (String method : serviceMethods) {
                ...
                if (CollectionUtils.isEmpty(methodInvokers)) {
        ...
        for (String method : Sets.newHashSet(newMethodInvokerMap.keySet())) {
            List<Invoker<T>> methodInvokers = newMethodInvokerMap.get(method);
            methodInvokers.sort(InvokerComparator.getComparator());
    ...
    private void destroyAllInvokers() {
        Map<String, Invoker<T>> localUrlInvokerMap = this.urlInvokerMap; // local reference
        if (!CollectionUtils.isEmpty(localUrlInvokerMap)) {
            for (Invoker<T> invoker : Lists.newArrayList(localUrlInvokerMap.values())) {
                try {
                    invoker.destroy();
                } catch (Throwable t) {
                    LogHelper.warn(logger, "Failed to destroy service " + serviceKey + " to provider " + invoker.getUrl(), t);
                }
            }
     ...
     *
     * @param newUrlInvokerMap new url invoker map
     * @param oldUrlInvokerMap old url invoker map
     */
            ...
            for (String url : deleted) {
                if (url != null) {
                    ...
                    if (invoker != null) {
                        try {
                            invoker.destroy();
                            LogHelper.debug(logger, "destroy invoker[" + invoker.getUrl() + "] success. ");
                        } catch (Exception e) {
                            LogHelper.warn(logger, "destroy invoker[" + invoker.getUrl() + "] failed. " + e.getMessage(), e);
                        }
                    }
                }
            }
    ...
    public List<Invoker<T>> doList(Invocation invocation) {
        ...
        if (!CollectionUtils.isEmpty(localMethodInvokerMap)) {
            ...
            if (!CollectionUtils.isEmpty(args) && args[0] != null
        ...
        return invokers == null ? Lists.newArrayListWithCapacity(0) : invokers;
    }
