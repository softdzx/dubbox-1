...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
    ...
    protected List<URL> loadRegistries(boolean provider) {
        checkRegistry();
        List<URL> registryList = Lists.newArrayList();
        if (!CollectionUtils.isEmpty(registries)) {
            for (RegistryConfig config : registries) {
                ...
                if (!Strings.isNullOrEmpty(sysAddress)) {
                    address = sysAddress;
                }
    ...
    protected URL loadMonitor(URL registryURL) {
        if (monitor == null) {
            ...
            if (!Strings.isNullOrEmpty(monitorAddress)
                    || Strings.isNullOrEmpty(monitorProtocol)) {
    ...
    protected void checkStubAndMock(Class<?> interfaceClass) {
        if (ConfigUtils.isNotEmpty(local)) {
            Class<?> localClass = ConfigUtils.isDefault(local) ? ReflectUtils.forName(interfaceClass.getName()
                    + "Local") : ReflectUtils.forName(local);
        ...
        if (ConfigUtils.isNotEmpty(stub)) {
            Class<?> localClass = ConfigUtils.isDefault(stub) ? ReflectUtils.forName(interfaceClass.getName()
                    + "Stub") : ReflectUtils.forName(stub);
            if (!interfaceClass.isAssignableFrom(localClass)) {
                throw new IllegalStateException("The local implemention class " + localClass.getName()
                        + " not implement interface " + interfaceClass.getName());
            }
        ...
        if (ConfigUtils.isNotEmpty(mock)) {
            if (mock.startsWith(Constants.RETURN_PREFIX)) {
                ...
                try {
                    ...
                    throw new IllegalStateException("Illegal mock json value in <dubbo:service ... mock=\""
                            + mock + "\" />");
                }
            } else {
                Class<?> mockClass = ConfigUtils.isDefault(mock) ? ReflectUtils.forName(interfaceClass.getName()
                        + "Mock") : ReflectUtils.forName(mock);
